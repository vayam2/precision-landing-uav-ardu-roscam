...
class PrecisionLand:
    def __init__(self):
        rospy.init_node('kinStk', anonymous=True)
        self.bridge = CvBridge()
        self.image_sub = rospy.Subscriber('/roscam/cam/image_raw', Image, self.processFrame)
        ...
        self.takeoff_alti = 15  # unit: metres
        self.arm_and_takeoff(self.takeoff_alti) 

        # self.lidar_alti = self.vehicle.rangefinder.distance   # unit: metres
        self.lidar_alti = self.vehicle.location.global_relative_frame.alt   # unit: metres
        self.land_alti = 5   # unit: metres
        # self.flag = 1
    ...
    def processFrame(self, data):
        ...
        if ids is not None:
            ...
            # if self.lidar_alti >= 0 and self.flag == 1:
            if self.lidar_alti >= 0:
                self.send_velocity(2, 0, 0)
